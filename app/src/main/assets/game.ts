/* =========================================================
   ZODIAC FUSION ARENA - CORE TYPESCRIPT GAME ENGINE (game.ts)
   ========================================================= */

export interface ZodiacMonth {
  id: number;
  name: string;
  glyph: string;
  element: string;
...
export interface EnemyEntity {
  name: string;
  month: { id: number; name: string; glyph: string; color?: string; tech?: string };
  animal: ChineseAnimal | { name: string; emoji?: string; icon: string; trait: string; speedMult?: number; index: number };
  x: number;
  y: number;
  radius: number;
  hp: number;
...
export interface GameState {
  ...
  currentGate: number;
  conqueredRealms: Set<number>;
  unlockedRealms: Set<number>;
  totalDefeats: number;
  campaignStartTime: number;
  constellationShards: number;
  ...
  isPractice: boolean;
  codexDates: Record<string, string>;
  viewWidth: number;
  viewHeight: number;
}

// 12 Constellation Months (Sign, Name, Glyph, Element, Active Technique)
export const ZODIAC_MONTHS: readonly ZodiacMonth[] = [
  ...
  { id: 2, name: "Aquarius", glyph: "♒", element: "Air", tech: "Pushing Water Fan", desc: "Releases a 90° wave blast that knocks back and slows foes." },
...
// 12 Chinese Zodiac Animals (repeating cycle anchored to 2020 = Rat)
export const CHINESE_ANIMALS: readonly ChineseAnimal[] = [
  { index: 0, name: "Rat", icon: "🐀", trait: "Quick sidestep & item magnet", effect: "rat_dodge" },
  { index: 1, name: "Ox", icon: "🐂", trait: "15% damage resistance & heavy dash", effect: "ox_guard" },
  { index: 2, name: "Tiger", icon: "🐅", trait: "3rd weapon combo hit deals +100% damage", effect: "tiger_fury" },
  { index: 3, name: "Rabbit", icon: "🐇", trait: "Grants a second rapid dash charge", effect: "rabbit_dash" },
  { index: 4, name: "Dragon", icon: "🐉", trait: "Dashes leave a trail of burning astral fire", effect: "dragon_fire" },
  { index: 5, name: "Snake", icon: "🐍", trait: "Poison immunity & evasive slither dash", effect: "snake_evade" },
  { index: 6, name: "Horse", icon: "🐎", trait: "+25% sustained movement speed", effect: "horse_speed" },
  { index: 7, name: "Goat", icon: "🐐", trait: "Restores 20 HP after clearing each phase", effect: "goat_heal" },
  { index: 8, name: "Monkey", icon: "🐒", trait: "Leaves a starlight decoy upon dodging", effect: "monkey_decoy" },
  { index: 9, name: "Rooster", icon: "🐓", trait: "Sign techniques deal +30% critical damage", effect: "rooster_power" },
  { index: 10, name: "Dog", icon: "🐕", trait: "Divine Aegis: absorbs first hit in each phase", effect: "dog_shield" },
  { index: 11, name: "Pig", icon: "🐖", trait: "Landing full 3-hit combo heals 6 HP", effect: "pig_lifesteal" }
];
...
export class AudioController {
  ...
  public playTone(freq: number, type: OscillatorType, duration: number, vol: number = 0.2): void {
  ...
  public playStrike(comboIndex: number): void {
  ...
  public playShadowTransform(): void {
...
export const state: GameState = {
  playerMonth: 8,
  playerYear: 2000,
  playerAnimalIndex: 4, // Dragon
  guardianTitle: "Leo–Dragon Guardian Prime",
  currentRealmIndex: 2, // Tiger Realm
  currentGate: 1,
  conqueredRealms: new Set([0, 1]),
  unlockedRealms: new Set([0, 1, 2]),
  totalDefeats: 0,
...
// Helper: Compute Chinese Animal from Year
export function getAnimalIndexForYear(year: number): number {
  return ((year - 2020) % 12 + 12) % 12;
}

// Helper: Compute Western Zodiac Month from Month
export function getZodiacMonth(monthId: number): ZodiacMonth {
  return ZODIAC_MONTHS[((monthId - 1) % 12 + 12) % 12];
}

// Particle & Floating Text helpers
export function addParticle(x: number, y: number, color: string, size: number = 3, vx: number = 0, vy: number = 0): void {
  state.particles.push({ x, y, color, size, vx, vy, life: 1.0 });
}

export function addFloatingText(x: number, y: number, text: string, color: string = "#FFF"): void {
...
// Touch Action Binding Helper
export function bindTouchAction(target: string | HTMLElement | null, actionCallback?: (e: Event) => void): void {
...
// Main Engine Initializer & Manager
let canvas: HTMLCanvasElement | null = null;
let ctx: CanvasRenderingContext2D | null = null;
let lastTime = performance.now();
let realmFxRaf: number | null = null;
let realmStars: Array<{ x: number; y: number; radius: number; alpha: number; speed: number }> = [];
let realmSparks: Array<{ x: number; y: number; vx: number; vy: number; life: number; color: string }> = [];
...
export function spawnGateEnemy(): void {
  const realmAnimal = CHINESE_ANIMALS[state.currentRealmIndex % 12];

  // 1. PRACTICE DUMMY SPARRING ENEMY
  if (state.isPractice) {
    state.enemy = {
      name: "Astral Training Dummy",
      month: { id: 0, name: "Astral", glyph: "✦", color: "#FFD700", tech: "Light Spark" },
      animal: { name: "Dummy", emoji: "🎯", icon: "🎯", trait: "Stationary Practice Target", speedMult: 0, index: 0 },
      x: 0,
      y: -85,
      radius: 22,
      hp: 90,
    ...
    if (bossName) bossName.textContent = "Astral Training Puppet (Practice)";
    const bossPhase = document.getElementById('hud-boss-phase');
    if (bossPhase) {
      bossPhase.textContent = "Practice Target";
    ...
    if (realmNameEl) realmNameEl.textContent = "Sparring Sanctum";
    const gateNumEl = document.getElementById('hud-gate-num');
    if (gateNumEl) gateNumEl.textContent = "TUT";
    return;
  }

  // 2. GATE 13: YEARLY REALM CHIMERA BOSS / FINAL SHADOW CHIMERA
  if (state.currentGate >= 13) {
    const isFinalTitan = (state.currentRealmIndex === 11);
    const chimeraTitle = isFinalTitan ?
      "Chronos All-Zodiac Chimera Shadow Titan" :
      `${realmAnimal.name} Realm Chimera Boss`;
    ...
    const bossPhase = document.getElementById('hud-boss-phase');
    if (bossPhase) {
      bossPhase.textContent = isFinalTitan ? "FINAL TITAN PHASE" : "CHIMERA BOSS";
      bossPhase.className = "boss-phase-tag";
    }
    const bossHp = document.getElementById('hud-boss-hp');
    if (bossHp) {
    ...
    const realmNameEl = document.getElementById('hud-realm-name');
    if (realmNameEl) realmNameEl.textContent = isFinalTitan ? "The Astral Void" : `Apex of ${realmAnimal.name}`;
    const gateNumEl = document.getElementById('hud-gate-num');
    if (gateNumEl) gateNumEl.textContent = isFinalTitan ? "MAX" : "CHM";
    return;
  }

  // 3. REGULAR GATE 1-12 GUARDIAN
  ...
  const realmNameEl = document.getElementById('hud-realm-name');
  if (realmNameEl) realmNameEl.textContent = `Realm of the ${realmAnimal.name} 🗺️`;
  const gateNumEl = document.getElementById('hud-gate-num');
  if (gateNumEl) gateNumEl.textContent = String(state.currentGate).padStart(2, '0');
}

export function triggerShadowPhase(): void {
...
export function recordCodexEntry(monthId: number, animalIdx: number): void {
  if (monthId < 1 || monthId > 12) return;
  const code = `${monthId}_${animalIdx}`;
  state.unlockedCodex.add(code);
  state.codexDates[code] = new Date().toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
  try {
...
export function updateCodexCounter(): void {
...
export function performStrike(): void {
...
export function performSignTechnique(): void {
  const p = state.player;
  ...
  if (month.id === 8) {
    // Leo Solar Roar: wide cone burst
...
export function performHeal(): void {
...
export function onGateVictory(): void {
  audio.playFanfare();
...
export function updateShardsUI(): void {
...
export function updatePlayerHpUI(): void {
...
export function update(dt: number): void {
  ...
  if (p.signCooldown > 0) {
    p.signCooldown -= dt;
    const pct = Math.max(0, p.signCooldown / 6.0);
...
// 12-Realm Map & Transition Sequence Handlers
export function renderRealmMapGrid(illuminatingIdx: number = -1): void {
...
export function showRealmInfoPreview(idx: number): void {
  ...
  if (tagEl) {
    tagEl.textContent = isConquered
      ? '✦ REALM MASTERED ✦'
      : (isUnlocked ? '✦ REALM ACCESSIBLE ✦' : '✦ REALM SEALED 🔒 ✦');
  }
  const descEl = document.getElementById('new-realm-desc');
  if (descEl) {
    descEl.textContent = isConquered
      ? `All 12 monthly gates mastered! Passive trait: ${animal.trait}.`
      : (isUnlocked
        ? `12 monthly gates & shadow chimera challenge. Trait: ${animal.trait}.`
        : `Defeat earlier realm gates to shatter this celestial seal.`);
  }
}

export function getNodeCenter(idx: number): { x: number; y: number } {
  const fxCanvas = document.getElementById('realm-fx-canvas') as HTMLCanvasElement | null;
...
export function initRealmFxCanvas(fromIdx: number, toIdx: number): void {
  const fxCanvas = document.getElementById('realm-fx-canvas') as HTMLCanvasElement | null;
  if (!fxCanvas) return;
...
export function spawnRealmBurstParticles(nodeIdx: number): void {
...
export function triggerRealmUnlockSequence(fromRealm: number = state.currentRealmIndex, toRealm: number = (state.currentRealmIndex + 1) % 12): void {
  ...
  setTimeout(() => {
    ...
    if (navigator.vibrate) {
      try { navigator.vibrate([40, 80, 120]); } catch { /* Ignore */ }
    }
  }, 1800);
}

export function openRealmMapModal(): void {
  renderRealmMapGrid(-1);
...
export function enterUnlockedRealm(): void {
...
export function showVictoryScreen(isFinalTitan: boolean = false): void {
  const elapsedSec = Math.floor((Date.now() - state.campaignStartTime) / 1000);
  ...
  if (isFinalTitan) {
    if (vicBadge) vicBadge.textContent = "👑";
...
export function populateCodexGrid(): void {
  ...
  for (let m = 1; m <= 12; m++) {
    for (let a = 0; a < 12; a++) {
      ...
      slot.innerHTML = `
        <div class="codex-slot-icon">${isUnlocked ? month.glyph + animal.icon : '🔒'}</div>
...
export function showCodexDetail(month: ZodiacMonth, animal: ChineseAnimal, isUnlocked: boolean): void {
  ...
  if (emblem) emblem.textContent = isUnlocked ? (month.glyph + " " + animal.icon) : '🔒';
  ...
  if (lore) {
    lore.innerHTML = isUnlocked ?
      `<strong>Active Technique:</strong> ${month.tech}<br><strong>Animal Trait:</strong> ${animal.trait}<br><br>A celestial guardian channeling the starry aura of ${month.name} fused with the ${animal.name} spirit.` :
      `Conquer this guardian in the ${animal.name} realm or monthly gate to unlock its cel-shaded codex archives and recorded attributes.`;
  }
...
export function generateShareCard(): void {
  const cardCanvas = document.getElementById('share-card-canvas') as HTMLCanvasElement | null;
...
export function shareSocialResults(): void {
  const shareData = {
...
export function initGame(): void {
  canvas = document.getElementById('arena-canvas') as HTMLCanvasElement | null;
  if (canvas) {
  ...
  try {
    ...
    if (savedCodex) {
      JSON.parse(savedCodex).forEach((id: string) => state.unlockedCodex.add(id));
    }
  ...
  window.addEventListener('keydown', (e: KeyboardEvent) => {
    if (e.key === 'w' || e.key === 'ArrowUp') { state.joystick.active = true; state.joystick.dy = -1; }
    if (e.key === 's' || e.key === 'ArrowDown') { state.joystick.active = true; state.joystick.dy = 1; }
    if (e.key === 'a' || e.key === 'ArrowLeft') { state.joystick.active = true; state.joystick.dx = -1; }
    if (e.key === 'd' || e.key === 'ArrowRight') { state.joystick.active = true; state.joystick.dx = 1; }
  ...
  const yearInput = document.getElementById('input-year') as HTMLInputElement | null;

  const updateCreationPreview = () => {
    if (!monthSelect || !yearInput) return;
    const mVal = parseInt(monthSelect.value);
    const yVal = parseInt(yearInput.value) || 2000;
  ...
  bindTouchAction('btn-confirm-forge', () => {
    ...
    state.playerYear = parseInt(yearInput.value) || 2000;
...
// Auto-run if loaded in a browser
